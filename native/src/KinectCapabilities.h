...
typedef struct {
	...
	bool			hasHandGripReleaseSupport;
	...
	FREObject		asFREObject() {
		...
		FRENewObjectFromBool(hasHandGripReleaseSupport, &freObject);
        FRESetObjectProperty(capabilities, (const uint8_t*)"hasHandGripReleaseSupport", freObject, NULL);
